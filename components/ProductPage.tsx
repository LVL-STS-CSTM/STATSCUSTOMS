        ...
        <div className="bg-white min-h-screen font-sans text-[#111]">
            <div className="max-w-[1600px] mx-auto px-4 md:px-8 py-8 md:py-16">
                ...
                <button 
                    ...
                    className="mb-8 text-[10px] font-space-grotesk font-bold uppercase tracking-[0.2em] text-zinc-400 hover:text-black transition-colors flex items-center gap-2 group"
                >
                    ...
                    <div className="lg:col-span-4 flex flex-col h-full sticky top-24">
                        <header className="mb-8 border-b border-zinc-100 pb-8">
                            ...
                            <div className="flex items-center justify-end mt-6">
                                <button 
                                    ...
                                    className="flex items-center gap-2 text-[9px] font-space-grotesk font-bold uppercase tracking-[0.2em] border-b border-black pb-1 hover:text-zinc-600 hover:border-zinc-600 transition-all"
                                >
                        ...
                        <div className="mb-10">
                            <div className="flex justify-between items-center mb-4">
                                <label className="block text-[10px] font-space-grotesk font-bold uppercase tracking-[0.2em] text-zinc-400">Color</label>
                                <span className="text-[10px] font-space-grotesk font-bold uppercase tracking-[0.2em] text-black">{selectedColor?.name}</span>
                            </div>
                                ...
                                <div className="pb-4 text-sm leading-relaxed text-zinc-600 font-futura font-light">
                                    <ul className="space-y-2">
                                        <li className="flex justify-between">
                                            <span className="font-space-grotesk font-bold text-black uppercase text-[10px] tracking-widest">Category</span>
                                            <span>{product.categoryGroup} / {product.category}</span>
                                        </li>
                                        <li className="flex justify-between">
                                            <span className="font-space-grotesk font-bold text-black uppercase text-[10px] tracking-widest">Fit</span>
                                            <span>{product.gender}</span>
                                        </li>
                                        <li className="flex justify-between">
                                            <span className="font-space-grotesk font-bold text-black uppercase text-[10px] tracking-widest">Min Order</span>
                                    ...
                                    {selectedMaterials.length > 0 ? (
                                        selectedMaterials.map((m, idx) => (
                                            <div key={m.id} className={idx > 0 ? "pt-4 border-t border-zinc-50" : ""}>
                                                <h4 className="text-[10px] font-eurostile font-bold uppercase tracking-widest text-black mb-1">{m.name}</h4>
                                                <p className="text-xs text-zinc-400 mt-1 font-futura">{m.description}</p>
                                                <div className="flex flex-wrap gap-2 mt-3">
                                                    {m.features.map(f => (
                                                        <span key={f} className="px-2 py-1 bg-zinc-100 text-[9px] font-space-grotesk font-bold uppercase tracking-wider text-zinc-600">
                                                            {f}
                                                        </span>
                                                    ))}
                                                </div>
                                        ...
                                        <div>
                                            <h4 className="text-[10px] font-eurostile font-bold uppercase tracking-widest text-black mb-1">Premium Technical Fabric</h4>
                ...
                <section className="mt-32 border-t border-zinc-100 pt-16">
                    <h3 className="text-[10px] font-eurostile font-bold uppercase tracking-[0.4em] text-zinc-400 mb-12 text-center">Product Features</h3>
                ...
                {relatedProducts.length > 0 && (
                    <section className="mt-32 pt-20 border-t border-zinc-100">
                        <div className="flex justify-between items-end mb-12">
                            ...
                            <button onClick={() => onNavigate('catalogue')} className="hidden md:block text-[10px] font-space-grotesk font-bold uppercase tracking-[0.2em] border-b border-black pb-1 hover:text-zinc-600 hover:border-zinc-600 transition-colors">View All</button>
                        </div>
