...
async function signJwt(email: string, privateKey: string, scope: string) {
  ...
  try {
    ...
    throw new Error(`JWT_SIGN_FAILURE`);
  }
}

async function getAccessToken(env: Env) {
  if (!env.GOOGLE_SERVICE_ACCOUNT_EMAIL || !env.GOOGLE_PRIVATE_KEY) throw new Error("CONFIG_MISSING");
  ...
  if (!res.ok) throw new Error(`GOOGLE_AUTH_REJECTED`);
...
export const onRequestPost = async (context: { env: Env; request: Request }) => {
  ...
  try {
    if (!(await checkRateLimit(ip, 'quotes', 20, 3600, env))) {
        return new Response(JSON.stringify({ message: 'Rate limit: Too many attempts.' }), { status: 429, headers: SECURITY_HEADERS });
    }
    ...
    // Aggregate items for sheet summary columns
    ...
    items.forEach((item: any) => {
        ...
        if (itemQty > 0) {
            ...
            if (item.logoData) logoLinks.push(`Attached [Base64 Size: ${item.logoData.length}]`);
            if (item.designData) designLinks.push(`Attached [Base64 Size: ${item.designData.length}]`);
        }
    });
    ...
    /**
     * REQUESTED SHEET MAPPING:
     * A: Quote ID | B: Submission Date | C: Status | D: Name | E: email | F: Phone 
     * G: Company | H: Message | I: Items | J: color | K: qty | L: total | M: LOGO | N: DESIGN
     */
    const sheetBody = {
        values: [[ 
            quoteId, 
    ...
    if (!sheetRes.ok) throw new Error(`GOOGLE_SHEETS_API_ERROR`);
...
export const onRequestGet = async (context: { env: Env; request: Request }) => {
    ...
    try {
        ...
        const res = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${env.GOOGLE_SHEET_ID}/values/Quotes!A:O`, { headers: { 'Authorization': `Bearer ${accessToken}` } });
        const data: any = await res.json();
        const rows = data.values || [];
        if (rows.length < 2) return new Response(JSON.stringify([]), { status: 200, headers: SECURITY_HEADERS });
        
        // Match the updated column structure
        const quotes = rows.slice(1).map((row: any) => ({
            id: row[0], submissionDate: row[1], status: row[2],
            contact: { 
                name: row[3], email: row[4], phone: row[5], company: row[6], message: row[7]
            },
            summary: row[8],
            colors: row[9],
            totalQty: row[10],
...
export const onRequestPut = async (context: { env: Env; request: Request }) => {
    ...
    try {
        ...
        const accessToken = await getAccessToken(env);
        const res = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${env.GOOGLE_SHEET_ID}/values/Quotes!A:A`, { headers: { 'Authorization': `Bearer ${accessToken}` } });
        const data: any = await res.json();
        const rows = data.values || [];
